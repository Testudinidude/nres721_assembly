...
Let's jump right in and start assembling data. If you haven't already, navigate to the directory you'd like to work from:

```{bash, comment = NA, eval = FALSE}
cd [your working directory]
```

Next, we want to download the data we'll use. If you've already done this (e.g., with the directions in the red box at the top of this page), you can just move that `\data` directory to your working directory. If you haven't, no problem! We can download those data now. If you're in your working directory, the following command will download our data into a `\data` directory within your current working directory:
...
From within our working directory, we'll create a new assembly by entering:

```{bash, eval = FALSE}
ipyrad -n template
```

Then, look at this template `ipyrad` parameters file:
...
* `[8] [restriction_overhang]` : change this parameter to "CTAGA,AATTC,CATAGC"
* `[21] [min_samples_locus]` : change this parameter to "5"
* `[25] [trim_reads]` : change this parameter to "0, 120, 0, 120"
* `[27] [output_formats]` : change this parameter to "*"

Then, save the file as `params-ubrucei_denovo.txt`. Next, we'll assemble the data. This whole process isn't too computationally-intensive (i.e., it took ~40 minutes on my 16 GB RAM, 4-core Macbook), but we might not want to wait quite that long. Don't worry—there is an opportunity to assemble data in real-time later in this tutorial. Below, we'll break this down step-by-step.
...
<div class="alert alert-info">
  <strong>Discussion:</strong> What can you glean from this output? What is the distribution of our sequencing coverage?
</div>
...
Create a new parameters file here by copying our *de novo* parameters file.
